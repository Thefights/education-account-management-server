...
namespace Services.FasApplications.Management
{
    public class ManagementFasApplicationService(IUnitOfWork unitOfWork, FasApplicationMapper mapper) : IManagementFasApplicationService
    {
        ...
        public async Task<FasApplicationDetailsDTO> GetApplicationDetailsAsync(string applicationId, int adminSchoolId, CancellationToken cancellationToken = default)
        {
            ...
            var application = await repo.Query()
                ...
                .Include(a => a.StudentNationality)
                .Include(a => a.ParentNationality)
                .Include(a => a.FasScheme)
                    .ThenInclude(s => s.Tiers)
                .Include(a => a.FasScheme)
                    .ThenInclude(s => s.RequiredDocuments)
                .Include(a => a.Documents)
                .FirstOrDefaultAsync(a => a.ApplicationNumber == applicationId && a.SchoolStudent.SchoolId == adminSchoolId, cancellationToken);
